...
{
    /**
     * @brief Archive that writes into a binary file. Note that reads and writes must be in consistent order, since everything is read/written consecutively! The "name" of variables is not used!
     */
    class BinaryOutputArchive : public Concrete<BinaryOutputArchive, IOutputArchive>
    {
    public:
        /**
         * @brief Constructor.
         ...
         * @param path File path where to read the archive from.
         * @return Flag that is true when the archive was successfully opened.
         */
         ...
         * @param name Name of the variable to read from the archive.
         * @param value Value is written into this variable.
         */
    ...
    private:
        ...
        void processVector(const char* name, Eigen::Vector<TValue, Size>& value)
        {
            ...
            {
                Eigen::Index size = value.cols();
                mStream.write((char*)&size, sizeof(Eigen::Index));
            }
            mStream.write((char*)value.data(), sizeof(TValue) * Size);
        }

        /**
         * @brief Writes an Eigen::Matrix type
        ...
        {
            Eigen::Index rows = value.rows(), cols = value.cols();
            mStream.write((char*)&rows, sizeof(Eigen::Index));
            mStream.write((char*)&cols, sizeof(Eigen::Index));
            mStream.write((char*)value.data(), sizeof(TValue) * rows * cols);
        }
        ...
        {
            Eigen::Index rows = value.rows(), cols = value.cols(), nnz = value.nonZeros();
            mStream.write((char*)&rows, sizeof(Eigen::Index));
            mStream.write((char*)&cols, sizeof(Eigen::Index));
            mStream.write((char*)&nnz, sizeof(Eigen::Index));
            typedef Eigen::Triplet<TValue> T;
            std::vector<T> tripletList;
            tripletList.reserve(nnz);
            for (int k = 0; k < value.outerSize(); ++k)
                for (typename Eigen::SparseMatrix<TValue>::InnerIterator it(value, k); it; ++it)
                    tripletList.push_back(T(it.row(), it.col(), it.value()));
            mStream.write((char*)tripletList.data(), sizeof(T) * nnz);
        }
